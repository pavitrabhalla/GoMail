...
class SendEmailTest(ResourceTestCase):
    def setUp(self):
        ...
        self.docFile = SimpleUploadedFile("dummy_doc.docx", "This is a temporary MS doc file")
        self.attachments = {'file':('dummy_doc.docx', self.docFile), 
                'file':('dummy_pdf.pdf', self.pdfFile), 
                'file':('dummy_img.jpg', self.imgFile)} 
    ...
    def test_send_email_attachments_success_aws(self):
        ...
        c = Client()
        self.assertHttpCreated(c.post('/api/v1/email-service/send-email/',
